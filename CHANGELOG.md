...
- **`pestphp/pest` `^4.0` → `^5.0`**, **`pestphp/pest-plugin-laravel` `^4.0` → `^5.0`** (둘 다 `require-dev`). Pest 5 가 php `^8.4` 를 요구하는 것이 php 하한을 올린 이유입니다. PHPUnit 도 13.3 으로 함께 올라갑니다.
...
- **생산 의존은 하나도 바뀌지 않았습니다.** php 하한을 올려 새로 받게 된 패키지는 Pest 5 와 pest-plugin-laravel 5 뿐이고 둘 다 `require-dev` 입니다. 직접 의존 20개를 최신판과 전수 대조한 결과, 나머지 18개는 이미 php `^8.3` 에서 최신을 받고 있었습니다. 이번 상향은 기능 확보가 아니라 장기 정리 목적입니다.
- **패키지 테스트는 이제 Laravel 13 에서만 돌 수 있습니다.** `require` 의 `laravel/framework ^11.0 || ^12.0 || ^13.0` 은 그대로이며 소비자의 Laravel 11·12 사용에는 영향이 없습니다. 다만 이 저장소의 `vendor/bin/pest` 는 L13 으로만 해석되므로, L11·L12 호환성은 로컬 테스트로 검증되지 않습니다.
