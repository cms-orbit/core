    ...
    private static function catalog(): array
    {
        return [
            InstallLocale::Korean->value => [
                ...
                'step_frontend_build'                 => '프런트엔드 의존성을 설치하고 에셋을 빌드합니다... (시간이 걸릴 수 있습니다)',
                'frontend_deps_added'                 => 'package.json에 Orbit 프런트엔드 의존성을 추가했습니다: :packages',
                'frontend_npm_missing'                => 'npm을 찾을 수 없어 자동 빌드를 건너뜁니다. 수동으로 `npm install && npm run build` 를 실행하세요.',
                'frontend_install_running'            => 'npm install 실행 중...',
                'frontend_build_running'              => 'npm run build 실행 중...',
                'frontend_build_done'                 => '프런트엔드 에셋 빌드를 완료했습니다.',
                'frontend_build_failed'               => '프런트엔드 자동 빌드에 실패했습니다. 수동으로 `npm install && npm run build` 를 실행해 주세요.',
                'step_ai'                             => 'AI 에이전트 스킬을 배포합니다...',
                'step_boost_update'                   => 'Laravel Boost 가이드라인·스킬을 갱신합니다...',
                'boost_install_hint'                  => 'Laravel Boost가 설치되어 있지만 아직 초기 설정이 없습니다. 한 번 `php artisan boost:install`을 실행한 뒤, 이후 `orbit:install`/`orbit:sync`가 cms-orbit 패키지 Boost 리소스를 자동 갱신합니다.',
                'boost_packages_registered'           => 'Boost에 cms-orbit 패키지 :count개를 등록했습니다.',
                'step_satellites'                     => '선택한 Orbit 위성 패키지를 설치합니다...',
                'satellites_prompt'                   => '추가로 설치할 Orbit 패키지를 선택하세요 (스페이스로 토글, 엔터로 확인)',
                'satellites_hint'                     => 'saas/blog는 테넌시 설정이 필요해 여기서 제외됩니다.',
                'satellites_require_running'          => 'composer require 실행: :packages',
                'satellites_require_failed'           => '위성 패키지 설치에 실패했습니다. 수동으로 실행하세요: :command',
                'satellites_step_running'             => '후속 단계 실행: :command',
                'satellites_step_failed'              => '후속 단계에 실패했습니다. 수동으로 실행하세요: :command',
                'satellites_unknown'                  => '지원하지 않는 --with 패키지: :packages (announcement, popup, sendgo만 가능)',
                'entities_created'                    => '루트 /entities 디렉터리를 생성했습니다.',
                'orbit_provider_created'              => 'App\\Orbit\\OrbitProvider를 생성했습니다.',
                'orbit_provider_registered'           => 'bootstrap/providers.php 에 OrbitProvider를 등록했습니다.',
                'orbit_provider_register_manually'    => 'bootstrap/providers.php 를 찾지 못했습니다. :provider 를 직접 등록하세요.',
            ...
            InstallLocale::English->value => [
                ...
                'step_frontend_build'                 => 'Installing frontend dependencies and building assets... (this may take a while)',
                'frontend_deps_added'                 => 'Added Orbit frontend dependencies to package.json: :packages',
                'frontend_npm_missing'                => 'npm was not found; skipping the automatic build. Run `npm install && npm run build` manually.',
                'frontend_install_running'            => 'Running npm install...',
                'frontend_build_running'              => 'Running npm run build...',
                'frontend_build_done'                 => 'Frontend assets built successfully.',
                'frontend_build_failed'               => 'Automatic frontend build failed. Please run `npm install && npm run build` manually.',
                'step_ai'                             => 'Publishing AI agent skills...',
                'step_boost_update'                   => 'Refreshing Laravel Boost guidelines and skills...',
                'boost_install_hint'                  => 'Laravel Boost is installed but not configured yet. Run `php artisan boost:install` once; later `orbit:install` / `orbit:sync` will refresh cms-orbit Boost resources automatically.',
                'boost_packages_registered'           => 'Registered :count cms-orbit package(s) with Boost.',
                'step_satellites'                     => 'Installing selected Orbit satellite packages...',
                'satellites_prompt'                   => 'Select optional Orbit packages to install (space to toggle, enter to confirm)',
                'satellites_hint'                     => 'saas/blog are excluded here because they need tenancy setup.',
                'satellites_require_running'          => 'Running composer require: :packages',
                'satellites_require_failed'           => 'Satellite package install failed. Run manually: :command',
                'satellites_step_running'             => 'Running follow-up: :command',
                'satellites_step_failed'              => 'Follow-up step failed. Run manually: :command',
                'satellites_unknown'                  => 'Unsupported --with package(s): :packages (only announcement, popup, sendgo)',
                'entities_created'                    => 'Created the root /entities directory.',
                'orbit_provider_created'              => 'Created App\\Orbit\\OrbitProvider.',
                'orbit_provider_registered'           => 'Registered OrbitProvider in bootstrap/providers.php.',
                'orbit_provider_register_manually'    => 'Could not find bootstrap/providers.php. Register :provider manually.',
